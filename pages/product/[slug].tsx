...
interface Props {
	getEachProduct: Product;
	getProducts: Product[];
	slug: string;
}

const ProductPage = ({ getEachProduct, getProducts, slug }: Props) => {
	const { data: eachProduct, isLoading } = useQuery(
		['each-product', slug],
		() => getEachProduct
	);

	const queryClient = useQueryClient();
	const saveProductData: Product[] | undefined = queryClient.getQueryData([
		'products',
	]);
	const { data: productData } = useQuery(['products'], () => getProducts);

	const products =
		saveProductData === undefined ? productData : saveProductData;
...
export const getStaticProps: GetStaticProps = async ({
	...
	const getEachProduct = await client.fetch(query);
	const getProducts = await client.fetch(productsQuery);

	const queryClient = new QueryClient();

	await Promise.all([
		queryClient.prefetchQuery(['each-product', slug], getEachProduct),
